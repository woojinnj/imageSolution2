...
from db_info04 import db_info_core, update_engine
from ui_basic04 import LogViewer
...
class Widgets(QWidget):
    ...
    def submit(self):
        try:
            db_info_core["username"]= self.text_username.text()
            db_info_core["password"] = self.text_password.text()
            db_info_core["database"] = self.text_database.text()
            db_info_core["host"] = self.text_host.text()
            db_info_core["port"] = self.text_port.text()
