...
def get_all_images2(
    ...
    count_sql = text("""
        ...
        WHERE (:q = '' OR e.code ILIKE '%' || :q || '%')
          AND (:status = '' OR e.state = :status)
    """)
    total_rows_code = db.execute(count_sql, {"q": q, "status": status}).scalar()
    ...
    if aq:
        filtered_core_results = []
        for row in core_results:
            ed_code = row[1]
            if article_map.get(ed_code) is not None:
                filtered_core_results.append(row)
        total_rows = len(filtered_core_results)
        total_pages = (total_rows // per_page) + (1 if total_rows % per_page > 0 else 0)
        # 수동 페이징 처리 (Python)
        start_index = (page - 1) * per_page
        end_index = start_index + per_page
        display_results = filtered_core_results[start_index:end_index]
    else:
        total_rows = total_rows_code
        total_pages = (total_rows // per_page) + (1 if total_rows % per_page > 0 else 0)
        display_results = core_results

    # HTML 생성
    if display_results:
        html = "<div class='image-gallery'>"
        for state, ed_code, _, _, content in display_results:
            ...
            if content is not None and content != "string":
                try:
                    ...
                    encoded_image = ""
            if encoded_image:
                image_html = f'<img data-src="data:image/jpeg;base64,{encoded_image}" alt="Image-{ed_code}" style="max-width:300px; display:none;">'
            else:
                image_html = '<p>이미지 없음</p>'
    ...
    <html>
        <head>
            ...
            <style>
                ...
                .article-data {{
                    ...
                    word-wrap: break-word;    /* 단어가 너무 길어도 강제로 줄바꿈 */
            ...
            <script>
                function goToPage(page) {{
                    ...
                    let articleQuery = document.getElementById("article-search-box").value;
                    window.location.href = `/image/all2?q=${{query}}&aq=${{articleQuery}}&status=${{status}}&page=${{page}}&per_page={per_page}`;
                }}
                function changePerPage() {{
                    ...
                    let status = document.getElementById("status-select").value;
                    let articleQuery = document.getElementById("article-search-box").value; 
                    window.location.href = `/image/all2?q=${{query}}&aq=${{articleQuery}}&status=${{status}}&page=1&per_page=${{selectedPerPage}}`;
                }}
                function searchImages() {{
                    let query = document.getElementById("search-box").value;
                    let status = document.getElementById("status-select").value;
                    let articleQuery = document.getElementById("article-search-box").value;  
                    window.location.href = `/image/all2?q=${{query}}&aq=${{articleQuery}}&status=${{status}}&page=1&per_page={per_page}`;
                }}
                ...
                function openAll() {{
                    ...
                    containers.forEach(function(container) {{
                        var dataDiv = container.querySelector(".article-data");
                        if (dataDiv) {{
                            dataDiv.style.display = "block"; // 데이터 영역 보이기
                            container.classList.add("highlight"); // 하이라이트 적용
                        }}
                        var img = container.querySelector("img");
                        if (img) {{
                            if (!img.getAttribute('src')) {{
                ...
                function closeAll() {{
                    ...
                    containers.forEach(function(container) {{
                        var dataDiv = container.querySelector(".article-data");
                        if (dataDiv) {{
                            dataDiv.style.display = "none"; // 데이터 영역 숨기기
                            container.classList.remove("highlight"); // 하이라이트 제거
                        }}
                         var img = container.querySelector("img");
                         if (img) {{
        ...
        <body>
            ...
            <div class="search-container">
                ...
                <input type="text" id="article-search-box" placeholder="Article 검색..." value="{aq}" onkeyup="if(event.keyCode === 13) searchImages()">
            ...
            <div class="global-controls" style="text-align: center; margin: 10px;">
    ...
    logging.info(f"검색어 '{q}', article 검색어 '{aq}', 상태 '{status}' 적용 - 페이지 {page}")
    return HTMLResponse(content=full_html)


@fastapi_app.get("/image/all2_b",
                 response_class=HTMLResponse)  # all2_b에서는 클릭했을 때 오른쪽에 데이터가 json 형태로 출력이 되고, 이미지도 같이 띄워주게, 지금 처럼 아래로 띄우는게 아니라 오른쪽 공간에 따로하는 것
def get_all_images2_b(
        page: int = 1,
        per_page: int = 10,
        q: str = "",  # 검색어 (code 검색)
        status: str = "",  # 상태 필터 (예: SUCCESS, TIMEOUT, UNASSIGNED)
        ...
        <head>
            ...
            <script>
                function goToPage(page) {{
                    ...
                    let status = document.getElementById("status-select").value;
                    let articleQuery = document.getElementById("article-search-box").value;
                    window.location.href = `/image/all2_b?q=${{query}}&aq=${{articleQuery}}&status=${{status}}&page=${{page}}&per_page={per_page}`;
                }}
                function changePerPage() {{
                    let selectedPerPage = document.getElementById("per-page").value;
                    let query = document.getElementById("search-box").value;
                    let status = document.getElementById("status-select").value;
                    let articleQuery = document.getElementById("article-search-box").value; 
                    window.location.href = `/image/all2_b?q=${{query}}&aq=${{articleQuery}}&status=${{status}}&page=1&per_page=${{selectedPerPage}}`;
                }}
                function searchImages() {{
                    let query = document.getElementById("search-box").value;
                    let status = document.getElementById("status-select").value;
                    let articleQuery = document.getElementById("article-search-box").value;  
                    window.location.href = `/image/all2_b?q=${{query}}&aq=${{articleQuery}}&status=${{status}}&page=1&per_page={per_page}`;
                }}
                function toggleBoth(element) {{
                    var container = element.parentElement;

                    var dataDiv = container.querySelector(".article-data");
                    var dataContent = dataDiv ? dataDiv.innerText : "";


                    if (dataContent.trim().startsWith("Data:")) {{
                    ...
                    try {{
                        ...
                        console.log("JSON 형식으로 변환 성공");

                    }} catch (e) {{
                        formattedData = dataContent;
                    }}
                    ...
                    if (img) {{
                        if (!img.getAttribute('src')) {{
                            img.src = img.getAttribute('data-src');
                            console.error("JSON 형식 변환 실패:", e);

                        }}
                    }}

                    var detailContainer = document.getElementById("detail-content");

                    var detailHTML = "<h2>" + element.innerText + "</h2>";
                    if (img) {{
                        var cloneImg = img.cloneNode(true);
                        cloneImg.style.display = "block";
                        detailHTML += "<div>" + cloneImg.outerHTML + "</div>";
                    }}
                        detailHTML += `
                    <textarea 
                        id="json-editor" 
                        style="width: 100%; height: 600px; font-size:12px; white-space: pre; font-family: monospace;"
                ...
                function saveJson() {{
                    let editedText=document.getElementById("json-editor").value;
                    let articleData;
                    try{{
                    ...
                    let payload={{
                        dataList:[articleData]
                   }};
                    fetch('http://1.220.2.154:15988/articles',{{
                        method: 'POST',
                        headers: {{
            ...
            <body>
              ...
              <div class="main-container" style="display:flex;">
                <!-- 왼쪽: 갤러리 영역 -->
                <div class="gallery-container" style="width:60%;">
                  <!-- 검색창 및 코드 -->
                  <div class="search-container">
                      <input type="text" id="search-box" placeholder="Code 검색..." value="{q}" onkeyup="if(event.keyCode === 13) searchImages()">
                      <input type="text" id="article-search-box" placeholder="Article 검색..." value="{aq}" onkeyup="if(event.keyCode === 13) searchImages()">
                ...
                <!-- 오른쪽 영역 -->
                <div class="detail-container" id="detail-content" style="width:40%; border-left:1px solid #ccc; padding:10px; overflow-y:auto;">
                  <h2>상세 정보</h2>
                  <p>이미지를 클릭하면 이 영역에 상세 정보가 표시됩니다.</p>
                </div>
              </div>
            </body>
...
async def save_articles(payload: DataListPayload, db: Session = Depends(get_db)):
    logging.info(f"저장 요청 받은 데이터: {payload.model_dump_json()}")
    return JSONResponse(content={"status": "success", "message": "데이터가 저장되었습니다."})
